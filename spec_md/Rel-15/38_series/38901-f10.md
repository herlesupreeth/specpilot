3GPP TR 38.901 V15.1.0 (2019-09)
...
The additional modelling components in this subclause are to support
...
steps between Step 1 and Step 12 in Subclause 7.5.
...
Step 11 in Subclause 7.5. The additional loss, OL*~n~*(*f~c~*) for
cluster *n* at centre frequency *f~c~* is modelled as:
...
*- τ~n~* is the *n*-th cluster delay \[s\] in Step 11 in Subclause 7.5;
...
The modelling in this subclause applies only when the bandwidth *B* is
...
in Subclause 7.5) is updated to model individual rays. In this case, the
...
unequal ray powers are considered,* *are generated according to
Subclause 7.6.2.2. Otherwise, ray powers are equal within a cluster,
i.e.,* *for all m.* Note: this model is developed assuming plane wave
propagation.
...
resolution can be larger than what the fast fading model in Subclause
7.5 is designed to support. To model this effect, the following
modifications to Step 7 in Subclause 7.5 can be optionally used.
...
generated in various channel generation steps in Subclause 7.5, so that
they are spatially consistent for drop-based simulations. Alternatively,
this can be used together with Procedure B described in Subclause
7.6.3.2 for spatially consistent mobility simulations.
...
\- Optionally in case of large bandwidth as described in Subclause
...
powers/delays/angles of clusters are generated according to Subclause
7.6.3.1.
...
UTvelocity vector respectively with
...
are the cluster delays in \[s\] as in Step 11 in Subclause 7.5;
...
angles are within reasonable limits, Steps 5, 6, and 7 in Subclause 7.5
...
, , *are generated in Step 4 of Subclause 7.5.* Normalize the cluster
powers so that the sum of all cluster powers is equal to one, i.e.,
...
of multi-frequency simulations according to Subclause 7.6.5, the largest
...
procedure for random variables as mentioned in Subclause 7.6.3.1, by
...
NLOS status should be used in Step 2 in Subclause 7.5 if this advanced
...
Subclause 7.4.3. Here, the focus is on modelling aspects with respect to
spatial consistency. As described in Subclause 7.4.3, the penetration
loss deviation σ~p~ represents variations within and between buildings
of the same type. For spatial consistency this can be modeled as a
spatially consistent random variable with correlation distance 10m, see
Subclause 7.6.3.1. The \"building type\" is determined using a spatially
...
Subclause 7.6.3.1 on the same floor are generated in the spatial
...
coefficient generation described in Subclause 7.5 are performed twice,
once with the propagation condition in Step 2 set as LOS and once with
the propagation condition in Step 2 set as NLOS. The resulting channel
...
When blockage model is applied, the channel generation in Subclause 7.5
...
This subclause describes how to generate parameters to reflect
correlation across different frequencies for a BS-UT link, for
simulations involved with multiple frequencies.

For those simulations, the steps in Subclause 7.5 should be revised
...
of the model according to Subclause 7.5, since cluster delays and angles
...
The alternative method replaces Steps 5-7 in Subclause 7.5 with the
...
frequency, e.g. 2 GHz: DS~0~, ASD~0~, ASA~0~, ZSD~0~, ZSA~0~, the delay
and angular spreads determined according to Step 4 at a frequency of
interest: DS, ASD, ASA, ZSD, ZSA, and the number of clusters *N* from
Table 7.5-6.
...
Subclause 7.5 shall consider UT rotational motion.
...
of the clutter. A typical value of *p* is 0.2.
...
Characterization of EM interference sources is out of the scope of the
channel model.
...
by e.g., coefficient generation Step 10 and Step 11 in Subclause 7.5 or
generating TDL model using spatial filter from Subclause 7.7.4. Three
CDL models, namely CDL-A, CDL-B and CDL-C are constructed to represent
three different channel profiles for NLOS while CDL-D and CDL-E are
...
Subclause 7.7.3. Each CDL model can also be scaled in angles so that the
model achieves desired angle spreads, according to the procedure
described in Subclause 7.7.5.1.
...
given by Table 7.5-3. If angular scaling according to Subclause 7.7.5.1
is used, this is applied to the ray angles , The generation of AOD (),
ZSA (), and ZSD () follows a procedure similar to AOA as described
above.
...
the exception that all clusters are treated as \"weaker cluster\", i.e.
...
The TDL models described in Subclause 7.7.2 are generated from the CDL
...
scaled according to Subclause 7.7.5.1 prior to the filtering in order to
...
a\. by the dominant path with ![](./media/image627.png), where denotes
the CDL cluster power values

b\. Or an arbitrary direction

4\. Calculate TDL cluster power values ![](./media/image630.png) given
...
|                                  | noise                            |
+----------------------------------+----------------------------------+
|                                  | 3\) CDF of delay and angle       |
|                                  | spread (ASD, ZSD, ASA, ZSA)      |
...
Subclause 3.3.1 of \[14\] with the square root matrixbeing generated
using the Cholesky decomposition and the following order of the large
scale parameter vector: **s***~M~* = \[*s~K,~ s~DS~, s~ASD~, s~ASA,~
s~ZSD,~ s~ZSA~*\]*^T^*. Limit random RMS azimuth arrival and azimuth
departure spread values to 104 degrees, i.e., ASA= min(ASA ,104°), ASD =
min(ASD ,104°). Limit random RMS zenith arrival and zenith departure
spread values to 52 degrees, i.e., ZSA = min(ZSA,52°), ZSD =
min(ZSD,52°). For the parameter selection from Table 7.5-6, the LOS/NLOS
condition determined in Step 3 is applied.
...
> and time *t* is modelled in the same way as given in Subclause 7.6.4;
...
the *n*th path and
![](./media/image783.wmf){width="0.29097222222222224in"
...
#########  Annex B: Change history
  ...
  **Date**             **Meeting**   **TDoc**     **CR**   **Rev**   **Cat**   **Subject/Comment**                                                                                                                                                                                                                                          **New version**
  ...
  2019-09              RAN\#85       RP-191944    0023     \-        B         Addition of indoor industrial channel model                                                                                                                                                                                                                  15.1.0
  -------------------- ------------- ------------ -------- --------- --------- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ -----------------
