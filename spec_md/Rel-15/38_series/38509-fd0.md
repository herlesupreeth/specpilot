3GPP TS 38.509 V15.13.0 (2022-06)
...
5.6.1.3	Reception of RESET UE POSITIONING STORED INFORMATION message by UE	27
...
Figures 5.3.4.1-3 shows functional block diagrams of UE test loop function for the TMC entity and UE test loop mode A for the case when standalone NR is configured.
...
2&gt;	if the MeasObjectId signalled by the IE SS-RSRPB Measurement Config and ReportConfigNR with reportType set to periodical which is associated to this MeasObjectNR are configured to the UE by RRC Signalling
...
2&gt;	else if the MeasObjectId signalled by the IE SS-RSRPB Measurement Config is configured to the UE by RRC signalling but no ReportConfigNR with reportType set to periodical is associated to this MeasObjectNR:
