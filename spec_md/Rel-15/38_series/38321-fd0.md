...
value which is not configured, the MAC entity shall at least:
...
1\> not transmit on UL-SCH except for Msg3 as specified in clause
5.4.2.2;

1\> if the *ra-ResponseWindow* or the *ra-ContentionResolutionTimer* is
running:
...
C-RNTI (as specified in clauses 5.1.4 and 5.1.5):
...
Random Access procedure completion (as specified in clauses 5.1.4 and
...
The MAC entity may be configured by RRC with a beam failure recovery
procedure which is used for indicating to the serving gNB of a new SSB
or CSI-RS when beam failure is detected on the serving SSB(s)/CSI-RS(s).
Beam failure is detected by counting beam failure instance indication
from the lower layers to the MAC entity. If *beamFailureRecoveryConfig*
is reconfigured by upper layers during an ongoing Random Access
procedure for beam failure recovery, the MAC entity shall stop the
...
*BeamFailureRecoveryConfig* and the *RadioLinkMonitoringConfig* for the
...
\- *ssb-perRACH-Occasion*: *ssb-perRACH-Occasion* for the beam failure
recovery;

\- *ra-ResponseWindow*: the time window to monitor response(s) for the
beam failure recovery using contention-free Random Access Preamble;

\- *prach-ConfigurationIndex*: *prach-ConfigurationIndex* for the beam
failure recovery;

\- *ra-ssb-OccasionMaskIndex*: *ra-ssb-OccasionMaskIndex* for the beam
failure recovery;
...
\- *BFI\_COUNTER*: counter for beam failure instance indication which is
initially set to 0.
...
1\> if the Random Access procedure is successfully completed (see clause
5.1):
...
PDSCH of a Serving Cell by sending the TCI States
Activation/Deactivation for UE-specific PDSCH MAC CE described in clause
6.1.3.14. The configured TCI states for PDSCH are initially deactivated
...
of a Serving Cell by sending the TCI State Indication for UE-specific
PDCCH MAC CE described in clause 6.1.3.15.
...
Activation/Deactivation MAC CE described in clause 6.1.3.17. The
...
Activation/Deactivation MAC CE described in clause 6.1.3.18. The
configured spatial relation for a PUCCH resource is initially
deactivated upon (re-)configuration by upper layers and after
reconfiguration with sync.
...
subheader for fixed sized MAC CE, padding, and a MAC SDU containing UL
...
set to 1 indicates that the Buffer Size field for the logical channel
group i is reported. The LCG~i~ field set to 0 indicates that the Buffer
Size field for the logical channel group i is not reported. For the Long
...
NOTE: The number of the Buffer Size fields in the Long BSR and Long
...
Trigger States configured within *CSI-aperiodicTriggerStateList*, as
...
\- S~i~: If there is a PUCCH Spatial Relation Info with
...
PCH, BCH, and DL-SCH including BCCH.
...
subheader. The size of the LCID field is 6 bits;
...
Table 7.1-2: RNTI usage.
  ...
  NOTE: The usage of MCS-C-RNTI is equivalent to that of C-RNTI in MAC procedures (except for the C-RNTI MAC CE).                                                                              
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
*msg1-SubcarrierSpacing* and Table 4.2-1 in TS 38.211 \[8\], and the
preamble formats are given by *prach-ConfigurationIndex* and Tables
6.3.3.2-2 and 6.3.3.2-3 in TS 38.211 \[8\].
